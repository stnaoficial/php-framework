...
namespace Miscellaneous\Kernel;

use InvalidArgumentException;
use Miscellaneous\Kernel\Abstracts\ServiceProvider;
use Oraculum\Alias\Network as AliasNetwork;
...
final class Kernel extends PrimitiveObject
{
    ...
    private function setupAlias($alias)
    {
        switch ($alias->getName()) {
            case 'mode':
                if (in_array($alias->getValue(), ["dev", "develop", "development"])) $this->showErrors();
                break;
            case 'boot.files':
    ...
    {
        $file = new File(__DIR__ . "/resources/maps/alias-network.php");

        $network = AliasNetwork::fromArray($file->require()); 

        // Changes the global alias network instance.
        // This makes the remaining aliases work as expected.
        AliasNetwork::setInstance($network);
    }
    ...
    private function registerBaseServices()
    {
        ...
        $this->setService(new \Miscellaneous\Exception\Services\Handling);
        $this->setService(new \Miscellaneous\Cli\Services\Handling);

        $this->setService(new \Miscellaneous\Kernel\Services\Command);

        $this->setService(new \Miscellaneous\Http\Services\Routing);
        $this->setService(new \Miscellaneous\Http\Services\Command);

        $this->setService(new \Miscellaneous\Stub\Services\Command);
    }
