...
namespace Miscellaneous\Exception;

use Oraculum\Cli\Console;
use Oraculum\Cli\Support\Ansi as AnsiSupport;
use Oraculum\Exception\Abstracts\ExceptionHandler as AbstractExceptionHandler;
...
final class ExceptionHandler extends AbstractExceptionHandler
{
    ...
    protected function handle($exception)
    {
        ...
        $console->writeLine(AnsiSupport::format("Code", AnsiSupport::DECORATION_BOLD));
        $console->writeLine($exception->getCode(), 2);
        ...
        $console->writeLine($exception->getMessage(), 2);
    }
}